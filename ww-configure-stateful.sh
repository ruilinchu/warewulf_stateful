...
object modify -s FILESYSTEMS="mountpoint=/boot:dev=$BOOTP:type=ext4:size=500,mountpoint=swap:dev=$SWAPP:type=swap:size=$SWAPSIZE,mountpoint=/:dev=$ROOTP:type=ext4:size=fill" $NODE
EOF
